...
void ArrowRenderer::drawArrow(TurnType type) {
    ...
    switch (type) {
        ...
        case UTURN_MEDIAN: {
            // Draw upper half circle with individual points
            for (int angle = 0; angle < 180; angle += 5) {
                float rad = angle * M_PI / 180.0f;
                int x = 64 + (int)(15 * cos(rad));
                int y = 32 - (int)(15 * sin(rad));
        ...
        case ROUNDABOUT_EXIT: {
            // Draw full circle with individual points
            for (int angle = 0; angle < 360; angle += 5) {
                float rad = angle * M_PI / 180.0f;
                int x = 64 + (int)(12 * cos(rad));
                int y = 32 + (int)(12 * sin(rad));
                lv_canvas_set_px(canvas, x, y, lv_color_white());
            }
...
void ArrowRenderer::drawDistance(int distance) {
    ...
    // Set label position
    lv_obj_align(label, NULL, LV_ALIGN_IN_TOP_MID, 0, DISTANCE_Y);
    ...
    for (int i = 0; i < count; i++) {
        ...
        if (activeLanes[i]) {
            ...
            for (int angle = 0; angle < 360; angle += 10) {
                float rad = angle * M_PI / 180.0f;
                int px = x + (int)(radius * cos(rad));
                int py = y + (int)(radius * sin(rad));
                lv_canvas_set_px(canvas, px, py, lv_color_white());
            }
        }
    }
}
...
void ArrowRenderer::drawStreetName(const char* name) {
    ...
    int len = strlen(name);
    if (len > 12) {
        lv_obj_align(label, NULL, LV_ALIGN_IN_TOP_LEFT, 5, STREET_Y);
    } else {
        lv_obj_align(label, NULL, LV_ALIGN_IN_TOP_MID, 0, STREET_Y);
    }
